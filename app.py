...
def build_narrative(score, bias, strategy, risk, regime_name, signals, cot):
    by = {s["name"]: s for s in signals}
    dxy = ar_gold_bias(by.get("DXY", {}).get("bias", "neutral"))
    real = ar_gold_bias(by.get("Real Yield 10Y", {}).get("bias", "neutral"))
    vix = ar_gold_bias(by.get("VIX", {}).get("bias", "neutral"))
    hui = ar_gold_bias(by.get("HUI Index", {}).get("bias", "neutral"))
    ratio = ar_gold_bias(by.get("Gold/Silver Ratio", {}).get("bias", "neutral"))
    cot_txt = ar_gold_bias(cot.get("bias", "neutral"))

    return f"""ملخص أكاديمية فلو اليومي
...
COT Weekly Filter: {cot_txt}

الرواية المؤسساتية:
تُظهر القراءة الحالية أن الذهب يتحرك داخل سياق يومي {ar_bias(bias)}. يعتمد القرار الأساسي اليوم على توازن الدولار والعوائد الحقيقية مع شهية المخاطرة. إذا بقي DXY والعوائد تحت ضغط، تبقى التصحيحات الشرائية مفضلة. إذا عاد الدولار والعوائد للصعود مع ضعف الزخم، يصبح انتظار الارتداد أفضل من مطاردة السعر.

خطة العمل:
{strategy}. لا تعتمد على القراءة وحدها. انتظر توافقها مع MSS أو FVG أو Sweep واضح على الشارت.
...
report = build_narrative(daily_score, daily_bias, preferred_strategy, risk_level, regime_name, signals, cot)
st.text_area("تقرير عربي جاهز للنشر", report, height=360)
...
alert_text = f"Flow Academy Gold Terminal\nDaily Bias: {ar_bias(daily_bias)}\nScore: {fmt_num(daily_score, 1)}/100\nPlan: {preferred_strategy}\nConfidence: {fmt_num(confidence, 1)}%"
if tg_token and tg_chat:
    if st.button("Send Telegram Alert Now"):
        try:
            url = f"https://api.telegram.org/bot{tg_token}/sendMessage"
            resp = requests.post(url, json={"chat_id": tg_chat, "text": alert_text}, timeout=20)
            if resp.ok:
                st.success("تم إرسال التنبيه إلى Telegram.")
            else:
                st.error(f"فشل الإرسال: {resp.text[:200]}")
        except Exception as e:
            st.error(f"Telegram error: {e}")
else:
    st.info("أدخل Bot Token و Chat ID من القائمة الجانبية إذا تريد إرسال تنبيه يدوي.")
